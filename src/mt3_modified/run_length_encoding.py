# Modified from the original [Magenta MT3 library](https://github.com/Magenta/mt3). See README.md
...
from mt3_modified import event_codec
...
def decode_events(
  ...
  for token_idx, token in enumerate(tokens):
    try:
      ...
      print(f'Error decoding event index {token}: {e}')
      invalid_events += 1
      continue
    if event.type == 'shift':
      cur_steps += event.value
      # NOTE: I believe there was a bug in the original code here. 
      # It was:
      #
      #     cur_time = start_time + cur_steps / codec.steps_per_second
      #
      # which would force the cur_time to reset every time we transitioned between
      # note events and 'shift' events. To 'pick up where we left off', instead
      # of 'start_time' (always 0) I use state.current_time.
      ...
      try:
        ...
        print(f'Error decoding event {event}: {e}')
        invalid_events += 1
        # logging.info(
